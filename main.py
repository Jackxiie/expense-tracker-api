...
def perform_trend_analysis(expense_data):
    """
    Group expenses by day first, then calculate a simple rolling trend.
    This makes the graph much more stable and readable.
    """
    ...
    daily_expense.sort_values(by='date', inplace=True)

    if len(daily_expense) < 3:
        raise ValueError("Insufficient data for trend analysis. Please add expenses for at least 3 different days.")

    daily_expense['trend'] = daily_expense['amount'].rolling(
        window=3,
        min_periods=1
    ).mean()

    return {
        "dates": daily_expense['date'].dt.strftime('%Y-%m-%d').tolist(),
        "original": daily_expense['amount'].tolist(),
        "trend": daily_expense['trend'].tolist()
    }
